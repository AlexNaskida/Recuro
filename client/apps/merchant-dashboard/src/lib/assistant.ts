...
export function buildAssistantSystemPrompt(context: AssistantContext): string {
  return [
    ...
    "When the user asks for action, use the available tools only when the recommendation is specific and immediately actionable.",
    ...
    "If a plan action is suggested, make the recommendation first and then call the relevant tool.",
