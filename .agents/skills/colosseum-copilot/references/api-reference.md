...
- `limit <= 10`. `limit` controls the number of _documents_ returned; each document contributes up to `maxChunksPerDoc` result items, so total items returned can be `limit × maxChunksPerDoc`.
...
**Limit semantics:** `limit` controls the number of _documents_ returned. Each document can have up to `maxChunksPerDoc` chunks, so total result items can exceed `limit`.
